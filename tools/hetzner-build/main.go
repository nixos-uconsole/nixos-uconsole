...
Set discord_webhook_url in config to receive notifications on success/failure.`,
	...
	rootCmd.PersistentFlags().
		StringVar(&cfgFile, "config", "", "config file (default: $HOME/.config/hetzner-build/config.toml)")
	rootCmd.Flags().
	...
	if configPath != "" {
		if _, err := os.Stat(configPath); err == nil {
			if _, err := toml.DecodeFile(configPath, &cfg); err != nil {
				fmt.Fprintf(
					os.Stderr,
					"Warning: failed to parse config %s: %v\n",
					configPath,
					err,
				)
			}
...
func run(cmd *cobra.Command, args []string) error {
	...
	defer func() {
		if shouldCleanup && created {
			...
			if err := deleteServer(server.Name); err != nil {
				notify(
					false,
					"Failed to delete server "+server.Name+": "+err.Error(),
				)
	...
	// Create server
	log(
		"Creating server %s (trying %v across %v)...",
		server.Name,
		cfg.FallbackTypes,
	...
	log(
		"Starting release in tmux (monitor with: ssh root@%s -t tmux attach)...",
		server.IP,
	)
...
func createServer(server *Server, created *bool) error {
	...
	for _, serverType := range types {
		for _, location := range locations {
			...
			if strings.Contains(errMsg, "resource_unavailable") ||
				strings.Contains(errMsg, "unavailable") {
				log(
					"%s unavailable in %s, trying next...",
					serverType,
					location,
				)
				lastErr = err
	...
	return fmt.Errorf(
		"all server type/location combinations unavailable: %w",
		lastErr,
	)
}
...
func waitForTmux(server *Server) error {
	...
	for i := 0; i < 720; i++ {
		out, _ := sshOutput(
			server,
			"tmux has-session -t release 2>/dev/null && echo running || echo done",
		)
		status := strings.TrimSpace(out)
		if strings.Contains(status, "done") {
			exitCode, _ := sshOutput(
				server,
				"cat /tmp/release-exit-code 2>/dev/null || echo 1",
			)
...
func checkSSHAgent() error {
	...
	if err != nil {
		return fmt.Errorf(
			"no SSH keys in agent. Run: ssh-add ~/.ssh/id_ed25519",
		)
	}
	if !strings.Contains(out, "ED25519") && !strings.Contains(out, "ed25519") {
		return fmt.Errorf(
			"no ed25519 key in agent. Run: ssh-add ~/.ssh/id_ed25519",
		)
	}
	return nil
}
